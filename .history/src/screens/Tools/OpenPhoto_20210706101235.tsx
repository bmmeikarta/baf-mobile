...
import { ScrollView, StyleSheet, TouchableOpacity, View, Text, Image, Platform, Alert } from 'react-native'
...
const OpenPhoto = () => {
    ...
    const deletePhoto = () => {
        const data = Photo.mode === 'photo' 
        ? photoCheck.filter((val, index) => !selectPhoto.includes(index))
        : scanCheck.filter((val, index) => !selectPhoto.includes(index))
    ...
    React.useEffect(() => {
        if (Photo.photoID === 'openPhoto') {
            if (Photo.scanCapture) {
                const datanya:any = ([
                    ...scanCheck.filter(val => val !== Photo.scanCapture),
                    Photo.scanCapture
                ])
    
                console.log(datanya)
    
                setScanCheck(datanya)
    ...
    const onSubmitPhoto = () => {
        ...
        Alert.alert(
            "Submit Data",
            `Apakah anda ingin submit ${Photo.mode} ini ?`,
            [
                {
                    text: "Cancel",
                    onPress: () => { },
                ...
                },
                {
                    ...
                    onPress: () => {
                        if (Photo.mode === 'photo') {
                            ...
                            dispatch({ type: 'SET_LIST_SCAN', payload: scanCheck })
                        }
                        dispatch({ type: 'SET_PHOTO_ID', payload: 'openPhoto' })
    ...
    return <View style={{ flex: 1, backgroundColor: 'white' }}>
        ...
        <ScrollView style={{ height: '70%', paddingTop: 30 }}>
            <View style={styles.containerContent}>
                {photoCheck.length > 0 || scanCheck.length > 0
                    ? <FlatGrid
                        itemDimension={Photo.mode === 'photo' ? 100 : 130}
                        data={Photo.mode === 'photo' ? photoCheck : scanCheck}
                        spacing={3}
                ...
                ? <>
                    ...
                    <View>
                        <Button
                            ...
                            disabled={photoCheck.length === 0 && scanCheck.length === 0}
                        ></Button>
                    </View>
                </>
