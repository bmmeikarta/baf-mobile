...
const ZoneChoose = () => {
    ...
    const checkZoneStored = (zone: any) => {
        console.log(storedData.filter(val => val.baf_blocks === Master.schedule_now.unit_blocks &&
            val.baf_tower === Master.schedule_now.unit_tower && 
            val.detail.filter(valDet => 
                valDet.baf_floor === Master.choosedFloor.floor &&
                valDet.baf_zone === zone
            ).length > 0))
        const data = storedData.some(val => 
        val.baf_blocks === Master.schedule_now.unit_blocks &&
        val.baf_tower === Master.schedule_now.unit_tower && 
        val.detail.filter(valDet => 
            valDet.baf_floor === Master.choosedFloor.floor &&
            valDet.baf_zone === zone
        ...
        <View style={{
            ...
            <TouchableOpacity onPress={() => onChooseZone(3)} style={{ width: 50, height: 80 }}>
                <View style={{ width: 50, height: 80, justifyContent: 'center' }}>
                    ...
                    <Text style={{ position: 'absolute', left: 3, color: checkZoneStored(1) ? 'white' : '#fff'  }}>Zone 3</Text>
                </View>
            </TouchableOpacity>
                ...
                <View style={{ width: 50, height: 80, justifyContent: 'center' }}>
                    <Svg width="50" height="80">
                        <Rect
                            ...
                            width="50"
                            height="80"
                            fill={checkZoneStored(1) ? "grey" : "green"}
                        />
                        <Text style={{ position: 'absolute', color: checkZoneStored(1) ? 'white' : '#fff', top: 30, left: 3 }}>Zone 4</Text>
                    </Svg>
                </View>
            </TouchableOpacity>
