...
import { Text, Header, Icon, Button, CheckBox, Badge } from "react-native-elements";
...
const BAFReport = () => {
    ...
    React.useEffect(() => {
        ...
        if (Master.photoGroupModal) {
            console.log('masuk sini')
            if (Master.photoGroupModal === 'yes') {
                setReportModal(true)
            } else {
        ...
        dispatch({ type: 'SET_INSERT_HISTORY', payload: [] })
    }, [
        JSON.stringify(Master.schedule_now),
        JSON.stringify(Master.categoryList),
        JSON.stringify(Master.photoCapture),
        JSON.stringify(Master.photoGroupModal)
    ])

    React.useEffect(() => {
        if (Master.bafChooser) {
            console.log(Master.bafChooser)
            // submitBAF(Master.bafChooser, Master.bafChooser.choosedStat)
        }
    }, [
        JSON.stringify(Master.bafChooser)
    ])

    const choosedBAF = (data: any) => {
        if (data.statusLabel === 'ok') {
            setChoosedBAFTemp([...choosedBAFTemp, data.id])
            submitBAF(data)
        } else {
    ...
    const removeBAF = (data: any) => {
        const check = Master.historyStore.filter(val => val.id !== data.id)
        setTempStore(check)
        dispatch({ type: 'SET_INSERT_HISTORY', payload: check })
    }
            ...
            [
                ...
                },
                {
                    ...
                    onPress: () => {
                        ...
                        //     setReportModal(false)
                        //     dispatch({ type: 'SET_PHOTO_GROUP_MODAL', payload: 'no' })
                        // })
                    },
                },
            ],
    ...
    return (
        <View style={styles.container}>
            ...
            <Text>{JSON.stringify(tempStore)}</Text>
