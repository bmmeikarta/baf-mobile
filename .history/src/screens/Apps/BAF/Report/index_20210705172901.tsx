...
const Report = () => {
    ...
    React.useEffect(() => {
        if (Master.mkrt_unit && Master.mkrt_unit.length !== 0) {    
    ...
    const checkSchedule = (data: any) => {
        const sch = scheduleUser.floor.some(fil => fil.floor == data.floor && 
            fil.tower == data.tower && 
            fil.blocks == data.blocks)

        return sch
    }

    const checkHist = (data: any) => {
        console.log(scheduleUser.floor)
        const hist = scheduleUser.floor.some(fil => 
            fil.floor == data.floor && 
            fil.tower == data.tower && 
            fil.block == data.blocks && 
            fil.hist.length > 0
        )
                                            ...
                                            renderItem={({ item, index }) => (
                                                <TouchableOpacity key={index} onPress={() => onOpenFloors(item)}>
                                                    <View style={[
                                                        ...
                                                        <Text style={styles.itemName}>{item.floor} {checkHist(item)}</Text>
                                                    </View>
                                                </TouchableOpacity>
                                            )}
                                        />
