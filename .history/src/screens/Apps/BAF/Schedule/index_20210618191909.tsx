                                    ...
                                    listFloor && listFloor.length > 0 ?
                                        listFloor.map((val, i) => (
                                            <ListItem
                                                ...
                                                containerStyle={{
                                                    backgroundColor: val.status === 'exists'
                                                        ? scheduleUser.floor && scheduleUser.floor.filter(fil => fil.floor == val.floor && fil.tower == val.tower && fil.block == val.block).length > 0
                                                            ? 'green'
                                                            : 'white'
                                                        : 'black'
                                                }}
                                            >
                                                ...
                                                <ListItem.Chevron
                                                    name={scheduleUser.floor && scheduleUser.floor.filter(fil => fil.floor == val.floor && fil.tower == val.tower && fil.block == val.block).length > 0
                                                        ? "lock-open"
                                                        : "lock"}
                                                    type='material'
                                                    color={scheduleUser.floor && scheduleUser.floor.filter(fil => fil.floor == val.floor && fil.tower == val.tower && fil.block == val.block).length > 0
                                                        ? 'white'
                                                        : 'black'
                                                    }
                                                />
