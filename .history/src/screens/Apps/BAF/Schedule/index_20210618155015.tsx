                                    ...
                                    <TouchableOpacity onPress={() => onChooseTower(Object.keys(listTower)[index])}>
                                        <View style={[styles.itemContainer, indexTower === Object.keys(listTower)[index] ? { backgroundColor: '#1abc9c', opacity: 0.5 } : { backgroundColor: '#1abc9c' }]}>
                                            <Badge
                                                ...
                                                value={scheduleUser.floor ? scheduleUser.floor.filter(fil => fil.tower == item && fil.block == indexBlocks).length : 0}
                                            />
                                    ...
                                    listFloor && listFloor.length > 0 ?
                                        listFloor.map((val, i) => (
                                            <ListItem
                                                ...
                                                containerStyle={{
                                                    backgroundColor: val.status === 'exists'
                                                        ? scheduleUser.floor && scheduleUser.floor.filter(fil => fil.floor == val.floor && fil.tower == val.tower && fil.block == val.blocks).length > 0
                                                            ? 'green'
                                                            : 'white'
                                                        : 'black'
                                                }}
                                            >
                                                ...
                                                <ListItem.Chevron
                                                    name={scheduleUser.floor && scheduleUser.floor.filter(fil => fil.floor == val.floor && fil.tower == val.tower && fil.block == val.blocks).length > 0
                                                        ? "lock-open"
                                                        : "lock"}
                                                    type='material'
                                                    color={scheduleUser.floor && scheduleUser.floor.filter(fil => fil.floor == val.floor && fil.tower == val.tower && fil.block == val.blocks).length > 0
                                                        ? 'white'
                                                        : 'black'
                                                    }
                                                />
