    ...
    return (
        <View style={styles.container}>
            ...
            <View style={{ padding: 10 }}>
                ...
                />
                {
                    listTower && Object.keys(listTower).length > 0
                    ? <>
                    <FlatGrid
                        itemDimension={130}
                            ...
                            <TouchableOpacity onPress={() => onChooseTower(Object.keys(listTower)[index])}>
                                <View style={[styles.itemContainer, indexTower === Object.keys(listTower)[index] ? { backgroundColor: '#1abc9c', opacity: 0.5 } : { backgroundColor: '#1abc9c' }]}>
                                    <Badge
                                        ...
                                        value={scheduleUser.floor ? scheduleUser.floor.filter(fil => fil.tower == item && fil.blocks == indexBlocks).length : 0}
                                    />
                                    <Text>{scheduleUser.floor.filter(fil => fil.tower == item && fil.blocks == indexBlocks).length}</Text>
                            ...
                            listFloor && listFloor.length > 0 ?
                                listFloor.map((val, i) => (
                                    <ListItem
                                        ...
                                        containerStyle={{
                                            backgroundColor: val.status === 'exists'
                                                ? scheduleUser.floor && scheduleUser.floor.filter(fil => fil.floor == val.floor && fil.tower == val.tower && fil.blocks == val.blocks).length > 0
                                                    ? 'green'
                                                    : 'white'
                                                : 'black'
                                        }}
                                    >
                                        ...
                                        <ListItem.Chevron
                                            name={scheduleUser.floor && scheduleUser.floor.filter(fil => fil.floor == val.floor && fil.tower == val.tower && fil.blocks == val.blocks).length > 0
                                                ? "lock-open"
                                                : "lock"}
                                            type='material'
                                            color={scheduleUser.floor && scheduleUser.floor.filter(fil => fil.floor == val.floor && fil.tower == val.tower && fil.blocks == val.blocks).length > 0
                                                ? 'white'
                                                : 'black'
                                            }
                                        />
