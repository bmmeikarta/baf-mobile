...
const AppsHome = () => {
    ...
    const renderView = (item: any) => {
        return <TouchableOpacity onPress={() => Actions[item.menu_url]()}>
            <View style={[styles.itemContainers, { backgroundColor: '#ff983d' }, item.menu_wide == 1 ? { width: windowWidth - 20 } : { width: 100 }]}>
                <Icon
                    style={styles.icon}
                    color='white'
                    type='font-awesome'
        ...
        <View style={styles.isi}>
            {
                Auth.parsedMenu.length > 0
                    ? <>
                        <Text>{JSON.stringify(Auth.parsedMenu.filter((val: any) => val.menu_wide == 1))}</Text>
                        <SectionGrid
                            itemDimension={150}
                            sections={Auth.parsedMenu.filter((val: any) => val.menu_wide == 1)}
                            style={styles.gridView}
                            renderItem={({ item, section, index }) => (renderView(item))}
                        ...
                        {/* <FlatGrid
                            itemDimension={100}
                            style={styles.gridView}
                            data={Auth.parsedMenu.filter((val: any) => val.menu_wide == 0)}
