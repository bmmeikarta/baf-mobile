...
const AppsHome = () => {
    ...
    const viewLayout = (data: any = []) => {
        return <Row style={{ paddingTop: '6%', paddingBottom: '6%', backgroundColor: 'white', borderBottomColor: 'lightgray', borderBottomWidth: 1 }}>
            <Col size={100} style={{backgroundColor: 'red'}}></Col>
        </Row>
    }
                ...
                Auth.parsedMenu.length > 0
                    ? <>
                        ...
                        <FlatGrid
                            itemDimension={150}
                            contentContainerStyle={styles.gridView}
                            data={Auth.parsedMenu}
                            renderItem={({ item }) => (
                                item.data.filter((val: any) => val.menu_wide == 0).map((valmap: any) => renderView(valmap))
                            )}
                            horizontal
                        />
                    </>
